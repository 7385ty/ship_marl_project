...
def evaluate_random_policy(env, scenario, num_episodes=20):
    """
    Evaluate random policy on one scenario.
...
if __name__ == "__main__":
    ...
    for scenario in scenarios:
        episode_results, summary = evaluate_random_policy(env, scenario, num_episodes=20)
    ...
    episode_csv_path = os.path.join(logs_dir, "random_policy_episode_results.csv")
    episode_df.to_csv(episode_csv_path, index=False)

    # Save summary-level results
    summary_df = pd.DataFrame(all_summary_results)
    summary_csv_path = os.path.join(logs_dir, "random_policy_summary.csv")
