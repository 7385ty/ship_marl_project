...
def plot_training_curves(log_df, figures_dir):
    ...
    plt.ylabel("Loss")
    plt.title("Minimal RL v3 - Training Loss Curves")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "minimal_rl_v3_loss_curves.png"), dpi=300)
    plt.close()
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Rate")
    plt.title("Minimal RL v3 - Evaluation Metrics")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "minimal_rl_v3_eval_curves.png"), dpi=300)
    plt.close()
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Return")
    plt.title("Minimal RL v3 - Evaluation Average Return")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "minimal_rl_v3_return_curve.png"), dpi=300)
    plt.close()


def save_checkpoint(checkpoint_dir, iteration, actor, critic, actor_optimizer, critic_optimizer, config):
    os.makedirs(checkpoint_dir, exist_ok=True)
    checkpoint_path = os.path.join(checkpoint_dir, f"minimal_rl_v3_iter_{iteration}.pt")
...
def main():
    ...
    env_config = {
        ...
        "danger_radius": 4.0,
        "danger_penalty_coef": 0.2,  # v3: reduced from 0.5
        "seed": 42,
    }
    ...
    log_csv_path = os.path.join(logs_dir, "minimal_rl_v3_train_log.csv")
