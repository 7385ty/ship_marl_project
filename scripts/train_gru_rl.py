...
from utils.ppo_utils import compute_returns, compute_advantages, normalize_advantages


def flatten_seq_batch(obs_seq, share_obs_seq, actions, log_probs, values, returns, advantages):
    """
    Flatten:
      obs_seq: (T, n_agents, seq_len, obs_dim)
    into:
      (T*n_agents, seq_len, obs_dim)
    """
    T, n_agents, seq_len, obs_dim = obs_seq.shape
    _, _, _, state_dim = share_obs_seq.shape
    _, _, action_dim = actions.shape

    batch = {
        "obs_seq": obs_seq.reshape(T * n_agents, seq_len, obs_dim),
        "share_obs_seq": share_obs_seq.reshape(T * n_agents, seq_len, state_dim),
...
def collect_rollout(env, actor, critic, scenario="head_on", rollout_steps=50, device="cpu"):
    buffer = RolloutBufferSeq()
    obs_seq, share_obs_seq, info = env.reset(scenario=scenario)

    for _ in range(rollout_steps):
        obs_tensor = torch.tensor(obs_seq, dtype=torch.float32, device=device)            # (n_agents, seq_len, obs_dim)
        share_obs_tensor = torch.tensor(share_obs_seq, dtype=torch.float32, device=device)  # (n_agents, seq_len, state_dim)
        ...
        next_obs_seq, next_share_obs_seq, rewards, dones, info = env.step(actions)

        buffer.add(
            obs_seq=obs_seq,
            share_obs_seq=share_obs_seq,
            actions=actions,
            rewards=rewards,
            dones=dones,
            log_probs=log_probs,
        ...
        share_obs_seq = next_share_obs_seq
...
def ppo_update(actor, critic, actor_optimizer, critic_optimizer, buffer, device="cpu", gamma=0.99, clip_eps=0.2):
    ...
    batch = flatten_seq_batch(
        obs_seq=data["obs_seq"],
        share_obs_seq=data["share_obs_seq"],
    ...
    obs_seq_tensor = torch.tensor(batch["obs_seq"], dtype=torch.float32, device=device)
    share_obs_seq_tensor = torch.tensor(batch["share_obs_seq"], dtype=torch.float32, device=device)
    ...
    new_log_probs, entropy = actor.evaluate_actions(obs_seq_tensor, actions_tensor)
    new_values = critic(share_obs_seq_tensor)
    ...
    total_actor_loss = actor_loss - 0.05 * entropy_mean
...
def evaluate_policy(env, actor, scenario="head_on", num_episodes=20, max_steps=60, device="cpu"):
    actor.eval()
    episode_results = []
    ...
    for _ in range(num_episodes):
        ...
        while not done and step_count < max_steps:
            obs_seq_tensor = torch.tensor(obs_seq, dtype=torch.float32, device=device)

            with torch.no_grad():
                mean, std = actor(obs_seq_tensor)
...
def plot_training_curves(log_df, figures_dir):
    ...
    plt.ylabel("Loss")
    plt.title("GRU RL - Training Loss Curves")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "gru_rl_loss_curves.png"), dpi=300)
    plt.close()
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Rate")
    plt.title("GRU RL - Evaluation Metrics")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "gru_rl_eval_curves.png"), dpi=300)
    plt.close()
    ...
    plt.xlabel("Iteration")
    plt.ylabel("Return")
    plt.title("GRU RL - Evaluation Average Return")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(figures_dir, "gru_rl_return_curve.png"), dpi=300)
    plt.close()


def save_checkpoint(checkpoint_dir, iteration, actor, critic, actor_optimizer, critic_optimizer, config):
    os.makedirs(checkpoint_dir, exist_ok=True)
    checkpoint_path = os.path.join(checkpoint_dir, f"gru_rl_iter_{iteration}.pt")
...
def main():
    ...
    train_env = MultiShipSeqWrapper(env_config=env_config, seq_len=seq_len)
    eval_env = MultiShipSeqWrapper(env_config=env_config, seq_len=seq_len)
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        log_row = {
            "iteration": iteration,
            "actor_loss": last_update_info["actor_loss"],
            "critic_loss": last_update_info["critic_loss"],
            "entropy": last_update_info["entropy"],
            "mean_return": last_update_info["mean_return"],
            "mean_advantage": last_update_info["mean_advantage"],
            "buffer_steps": last_update_info["buffer_steps"],
        ...
        print(f"Actor loss: {last_update_info['actor_loss']:.6f}")
        print(f"Critic loss: {last_update_info['critic_loss']:.6f}")
        print(f"Entropy: {last_update_info['entropy']:.6f}")
        print(f"Mean return: {last_update_info['mean_return']:.6f}")
        print(f"Buffer steps: {last_update_info['buffer_steps']}")
    ...
    log_csv_path = os.path.join(logs_dir, "gru_rl_train_log.csv")
