...
from utils.rule_policy import simple_rule_policy


def run_one_episode(env, scenario, episode_id):
    """
    Run one episode with simple rule-based policy.
...
def evaluate_rule_policy(env, scenario, num_episodes=20):
    """
    Evaluate rule-based policy on one scenario.
    """
    episode_results = []
...
if __name__ == "__main__":
    ...
    for scenario in scenarios:
        episode_results, summary = evaluate_rule_policy(env, scenario, num_episodes=20)
    ...
    episode_csv_path = os.path.join(logs_dir, "rule_policy_episode_results.csv")
    episode_df.to_csv(episode_csv_path, index=False)

    summary_df = pd.DataFrame(all_summary_results)
    summary_csv_path = os.path.join(logs_dir, "rule_policy_summary.csv")
    summary_df.to_csv(summary_csv_path, index=False)

    print("\n" + "=" * 70)
    print("Rule-policy CSV files saved successfully!")
